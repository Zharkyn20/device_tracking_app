...
class EmployeeRegistrationSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if user.is_company:
            name_values = Employee.objects.filter(company__company_user=user).values_list('name', flat=True)
        else:
            company = user.company
            name_values = Employee.objects.filter(company=company).values_list('name', flat=True)

        if validated_data['name'] in name_values:
            raise serializers.ValidationError(
                {"name": "This employee already exists."}
            )
